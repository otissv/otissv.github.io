...
const views = require('./src/views/views.js');
...
function getFiles (dir, fn, pre) {
  return fs.readdirAsync(dir).map(filename => {
    ...
    if (stat && stat.isDirectory()) {
      ...
      // call callback
      return fn && fn(`${pre || ''}${filename}`, filePath);
    }
  });
}
...
function run () {
  getFiles(directory, function (filename, filePath) {
    ...
    .then(data => {
      ...
      views.forEach(view => {
        buildViews({
          outputFolder: `${config.dist.views}/partials`,
          view: view,
          template: Handlebars.compile(getDataItem(views.partials).content)
        });
      });
    })
    .catch(function(e) {
